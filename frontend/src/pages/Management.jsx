...
const Management = () => {
  ...
  const stats = [
    { label: "Total Restaurants", value: isAdmin ? "12" : "1", icon: Store, color: "text-blue-600", bg: "bg-blue-50" },
    { label: "Active Menu Items", value: "48", icon: UtensilsCrossed, color: "text-orange-600", bg: "bg-orange-50" },
    { label: "Monthly Orders", value: "1,240", icon: BarChart3, color: "text-green-600", bg: "bg-green-50" },
  ];
    ...
    <div className="min-h-screen bg-[#F5F3EE]">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {stats.map((stat, idx) => (
            <div key={idx} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4 transition-transform hover:scale-[1.02]">
              <div className={`w-12 h-12 ${stat.bg} ${stat.color} rounded-xl flex items-center justify-center`}>
                <stat.icon size={24} />
              </div>
              <div>
                <p className="text-sm text-gray-400 font-medium">{stat.label}</p>
                <p className="text-2xl font-bold text-[#151515]">{stat.value}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="border-b border-slate-100">
            <nav className="flex px-6 overflow-x-auto">
              {["Overview", "Restaurants", "Menu Items", "Analytics"].map((tab) => (
                <button
                  key={tab}
          ...
          <div className="p-8">
            <div className="max-w-2xl mx-auto text-center py-12">
              ...
              <h2 className="text-2xl font-bold text-[#151515] mb-3">Upload Restaurant Data</h2>
              <p className="text-gray-500 mb-8">
                Drag and drop your restaurant data files or menu catalogs to start syncing with the BhojanGo platform.
              </p>
